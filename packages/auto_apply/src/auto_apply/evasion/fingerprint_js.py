...
def get_js_payload(settings: EvasionSettings) -> str:
    ...
    scripts.append("Object.defineProperty(navigator, 'webdriver', { get: () => false });")

    if settings.spoof_hardware_concurrency:
        scripts.append(f"Object.defineProperty(navigator, 'hardwareConcurrency', {{ get: () => {settings.spoof_hardware_concurrency} }});")
