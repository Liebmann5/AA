...
def apply_firefox_hardening(driver: WebDriver) -> None:
    ...
    """
    logger.info("Applying multi-patch JavaScript payload for MAXIMUM Firefox elusiveness.")

    try:
        driver.execute_script(_get_js_payload())
        logger.info("Successfully applied fortress-level JavaScript evasion patches.")
    except Exception as e:
        logger.error(f"Could not apply JavaScript patches to Firefox: {e}", exc_info=True)